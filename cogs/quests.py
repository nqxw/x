...
# v4.3 — transport uses modifyself Route + HeaderSpoofer (fingerprint-consistent)
import asyncio
import base64
import json
import os
...
def _quest_headers(token):
    """
    Kept for the aiohttp fallback path and for endpoints that don't
    route through the fork. Never used when _ForkTransport is active —
    the fork's HeaderSpoofer builds the real headers.
    """
...
class _ForkTransport:
    """
    Routes quest requests through modifyself's HTTPClient, which uses
    wreq + HeaderSpoofer. TLS fingerprint, x-super-properties, and
    user-agent are all generated by the fork and internally consistent —
    no manual header override.
    """
    def __init__(self, token, headers):
        # `headers` here is a HeaderSpoofer. We build it the same way
        # the fork's Client does, so the fingerprint matches the gateway
        # session running in the same process.
        try:
    ...
    async def request(self, method, url, headers=None, json=None):
        # strip the base URL — the fork prepends it
        path = url
        ...
        for attempt in range(3):
            try:
                ...
                if status:
                    # HTTPException from the fork — real status code
...
class _AiohttpTransport:
    """Fallback — no TLS impersonation, plain aiohttp."""
    def __init__(self, token, headers):
        import aiohttp
        self._aio = aiohttp
