...
class RPCCog:
    ...
    async def _get_asset_channel(self):
        """Return a text channel for asset storage.

        Discord blocks self-DMs (error code 50007), so a DM channel owned
        by the bot is not usable as a scratch sink. Set RPC_ASSET_CHANNEL_ID
        to any private text channel the account can post in.
        """
        if self._asset_channel is not None:
            return self._asset_channel

        raw = os.environ.get("RPC_ASSET_CHANNEL_ID", "").strip()
        if not raw:
            print("[RPC] RPC_ASSET_CHANNEL_ID not set — asset upload disabled")
            return None
        try:
            ch_id = int(raw)
        except ValueError:
            print(f"[RPC] RPC_ASSET_CHANNEL_ID invalid: {raw!r}")
            return None

        ch = self.client.get_channel(ch_id)
        if ch is None:
            try:
                ch = await self.client.fetch_channel(ch_id)
            except Exception as e:
                print(f"[RPC] fetch_channel({ch_id}) failed: {e}")
                return None

        if ch is None:
            print(f"[RPC] channel {ch_id} not accessible")
            return None

        self._asset_channel = ch
        return ch
    ...
    async def upload_asset(self, image_url: str):
        ...
        try:
            ...
            sink = await self._get_asset_channel()
            if sink is None:
            ...
            async with aiohttp.ClientSession() as s:
                async with s.get(image_url) as r:
                    if r.status != 200:
                        print(f"[RPC] asset fetch status {r.status}")
                        return None
                    ...
                    if msg.attachments:
                        ...
                        if m2:
                            ...
                            self._asset_cache[image_url] = key
                            return key
                        self._asset_cache[image_url] = new_url
                        return new_url
        except Exception as e:
